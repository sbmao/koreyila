    ...
    var halo = {
        ...
        menuSidebarMobileToggle: function() {
            ...
            $body.on('click', '.site-nav-mobile .list-menu .menu_mobile_link_2', function (e) {
                ...
                $parent.removeClass('is-hidden').toggleClass('is-open').removeClass('d-none');
                // e.preventDefault();
                // e.stopPropagation();
                // var $target = $(this);
                // var $target = $(this);
        ...
        initColorSwatch: function() {
            ...
            $doc.on('click', '.card .swatch-label',  async (event2) => {
                var $target = $(event2.currentTarget),
                    ...
                    // productJson = $target.data('json-product'),
                ...
                const productJson = (await halo.productJsonAJAX(handle)).product;
                const variantId = productJson.variants[0].id,
                newImage = productJson.image.src.replace(".jpg", "_x800.jpg"),
                selectedVariant = productJson.variants.find(
                ...
                // const selectedVariant = productJson
                ...
                product.find("a:not(.single-action):not(.number-showmore)").attr("href", `/products/${handle}?variant=${variantId}`)
                ...
                if (mediaList.length > 0) {
                    ...
                    if (newImage) {
                        product.find('.card-media img:first-of-type')
            ...
            $doc.on('click', '.item-swatch-more .number-showmore', (event2) => {
                if ($(event2.target).closest('.swatch').hasClass('show--more')) {
                    $(event2.target).closest('.swatch').removeClass('show--more');
                    $(event2.target).find('span:eq(0)').text('+');
                } else {
                    $(event2.target).closest('.swatch').addClass('show--more');
                    $(event2.target).find('span:eq(0)').text('-'); 
                }
            })
        },
        ...
        productJsonAJAX: async function (handle) {
          let result;
          try {
            return (
              (result = await $.ajax({
